...
// open wave output	
	...
	if(instance->c_fUseWaveOut) // if use audio output
	{
		...
		MMRESULT res;
		if ((res = waveOutOpen((LPHWAVEOUT)&instance->c_HWO_WaveOut, (UINT)wave_output_id, (LPCWAVEFORMATEX)&instance->c_wfx,
			(DWORD_PTR)_WaveOutProc, (DWORD_PTR)instance, (DWORD)CALLBACK_FUNCTION)) != MMSYSERR_NOERROR)
		{
